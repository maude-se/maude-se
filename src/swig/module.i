...
public:
	...
	%extend {
		...
		/**
		 * Get the metarepresentation in this module of a rule
		 * in (possibly) another module. This module must contain
		 * @c META-LEVEL.
		 *
		 * @param term Any term.
		 *
		 * @return The metarepresentation sort or null.
		 */
		EasyTerm* upSort(EasyTerm* term) {
			VisibleModule* otherModule = safeCast(VisibleModule*, term->symbol()->getModule());
			MetaLevel* metaLevel = getMetaLevel($self);
			...
			PointerMap qidMap;
			DagNode* result = metaLevel->upType(term->getDag()->getSort(), qidMap);
			return result == nullptr ? nullptr : new EasyTerm(result);
		}
